...
public class Information : IrcBot
{
 ...
 public void sendNamesToSrv()
 {
     ...
     writer.Flush();
    static ArrayList realUserList = new ArrayList();
...
public ArrayList users()
 {
     ...
     else if(rawPieces.Length >= 3)
     {
     
             Console.WriteLine(rawPieces[0]);
         userList.Add(rawPieces[0].Substring(1));
         for (int i = 0; i < rawPieces.Length - 1; i++)
         {
             userList.Add(rawPieces[i]);
         Console.WriteLine(rawPieces[i]);
         }
